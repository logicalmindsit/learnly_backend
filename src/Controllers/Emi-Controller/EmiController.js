...
export const getEmiStatus = async (req, res) => {
  try {
    ...
    console.error("Error getting EMI status:", error);
    res.status(500).json({
      success: false,
...
export const payOverdueEmis = async (req, res) => {
  try {
    ...
    console.error("Error in payOverdueEmis:", error);
    res.status(500).json({
      success: false,
...
export const getMonthlyDueAmount = async (req, res) => {
  try {
    ...
    console.error("Error getting monthly due amount:", error);
    res.status(500).json({
      success: false,
...
export const verifyEmiPayment = async (req, res) => {
  try {
    ...
    console.log("🔐 EMI Payment Verification:", {
      userId,
      razorpay_payment_id,
      razorpay_order_id,
      courseId,
    ...
    console.log("🔐 Payment allocation:", paymentAllocation);
    ...
    paymentAllocation.emisToPay.forEach((emi) => {
      ...
      if (!emi.emiId) {
        console.error("🔐 EMI object missing emiId:", emi);
      ...
      if (emiIndex !== -1) {
        ...
        console.log(`🔐 Marked EMI ${emi.month} (${emi.monthName}) as paid`);
      } else {
        console.error(`🔐 EMI not found in plan: ${emi.emiId}`);
    ...
    console.log("🔐 Updated EMI status after payment:", updatedStatus);
    ...
    if (updatedStatus.isCurrentOnPayments && emiPlan.status === "locked") {
      ...
      accessShouldBeUnlocked = true;
      console.log("🔐 Unlocking EMI plan access - user is current on payments");
    }
    ...
    console.log("🔐 Final EMI status after plan update:", {
      planStatus: emiPlan.status,
      hasAccessToContent: finalStatus.hasAccessToContent,
      isCurrentOnPayments: finalStatus.isCurrentOnPayments,
      overdueCount: finalStatus.overdueCount,
    });
    ...
    console.log("🔐 EMI payment verification completed successfully");
...
export const getUserEmiSummary = async (req, res) => {
  try {
    ...
    console.error("Error getting user EMI summary:", error);
    res.status(500).json({
      success: false,
...
export const getEmiDueAmounts = async (req, res) => {
  try {
    ...
    console.error("Error getting EMI due amounts:", error);
    res.status(500).json({
      success: false,
...
// Get complete EMI plan details for a course
export const getCompleteEmiPlan = async (req, res) => {
  try {
    const userId = req.userId;
    const { courseId } = req.params;
    ...
    // Get the complete EMI plan with populated user and course details
    const emiPlan = await EMIPlan.findOne({
      userId,
      courseId,
    })
      .populate("userId", "username studentRegisterNumber email mobile")
      .populate("courseId", "CourseMotherId coursename price courseduration");
    ...
    // Format the response to match the user's example
    const formattedEmiPlan = {
      _id: emiPlan._id,
      userId: emiPlan.userId._id,
      username: emiPlan.userId.username,
      studentRegisterNumber: emiPlan.userId.studentRegisterNumber,
      email: emiPlan.userId.email,
      mobile: emiPlan.userId.mobile,
      courseId: emiPlan.courseId._id,
      CourseMotherId: emiPlan.courseId.CourseMotherId,
      coursename: emiPlan.courseId.coursename,
      coursePrice: emiPlan.courseId.price.finalPrice,
      courseduration: emiPlan.courseId.courseduration,
      totalAmount: emiPlan.totalAmount,
      emiPeriod: emiPlan.emiPeriod,
      selectedDueDay: emiPlan.selectedDueDay,
      startDate: emiPlan.startDate,
      status: emiPlan.status,
      emis: emiPlan.emis,
      lockHistory: emiPlan.lockHistory || [],
      notifications: emiPlan.notifications || [],
      createdAt: emiPlan.createdAt,
      updatedAt: emiPlan.updatedAt,
    };

    res.status(200).json({
      success: true,
      data: formattedEmiPlan,
    });
  } catch (error) {
    console.error("Error getting complete EMI plan:", error);
    res.status(500).json({
      success: false,
      message: "Failed to get complete EMI plan details",
      error: error.message,
    });
  }
};
